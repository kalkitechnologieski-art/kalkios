    ...
    try {
      ...
      if (data.user) {
        const { data: profile } = await supabase
          ...
          .eq('id', data.user.id)
          .single()
        redirectBasedOnRole(profile?.role || 'client')
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-6">
      <div className="max-w-md w-full space-y-8">
        ...
        <p className="text-cyan-400/40 text-sm font-mono tracking-wider">
          {isTeamLogin ? '● TEAM ACCESS ●' : '● SECURE ACCESS ●'}
        </p>
        ...
        <div className="flex justify-center gap-3 text-xs text-cyan-400/20 font-mono">
          <button onClick={() => setIsTeamLogin(!isTeamLogin)} className="hover:text-cyan-400/60 transition">
            {isTeamLogin ? '← Individual login' : '🔐 Team login'}
          </button>
          <span>|</span>
          <button className="hover:text-cyan-400/60 transition">Forgot password?</button>
        </div>
      </div>
    </div>
  )
