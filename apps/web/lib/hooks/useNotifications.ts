...
// @ts-ignore
  ...
  // ─── Realtime subscription (SKIP on /chat) ──────────────────
  useEffect(() => {
    if (!user) return
    // Skip realtime on chat page to avoid conflict with chat subscriptions
    if (pathname === '/chat') {
      console.log('🔇 Skipping notifications realtime on /chat page')
      return
    }
    ...
    try {
      ...
      console.warn('Could not subscribe to notifications realtime:', error)
    }
