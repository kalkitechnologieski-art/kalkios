...
export async function POST(req: NextRequest) {
  ...
  const { messages, useSlm = false } = await req.json()
      ...
      try {
        ...
        await supabase.from('audit_logs').insert({
          ...
          details: {
            provider: 'webllm',
            success: true,
            timeMs: slmResponse.timeMs,
            tokens: slmResponse.tokens,
          },
        })

        return NextResponse.json({
          response: slmResponse.text,
          provider: 'webllm',
          usage: { total_tokens: slmResponse.tokens || 0 },
        })
      } catch (slmError) {
        logger.warn('SLM inference failed, falling back to cloud:', slmError)
        await reportErrorToAdmin(slmError as Error, 'webllm', { messages })
      }
    }
